...
sudo apt install build-essential git cmake \
    ...
    python3-packaging libuv1-dev libcairo2-dev \
...
sudo apt install libxcb-xkb-dev libxcb-xrm-dev \
    libxcb-cursor-dev libasound2-dev libpulse-dev \
    libjsoncpp-dev libmpdclient-dev \
    libcurl4-openssl-dev libnl-genl-3-dev &&
git clone --recursive https://github.com/polybar/polybar ~/builds/polybar &&
cd ~/builds/polybar &&
mkdir build && cd build && cmake .. && make -j17 &&
sudo make install
